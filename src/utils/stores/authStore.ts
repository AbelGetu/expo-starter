...
interface AuthState {
  ...
  logInAsVip: (credentials: LoginCredentials) => Promise<void>;
  logOut: () => Promise<void>;
...
export const useAuthState = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      logInAsVip: async (credentials) => {
        set({ isLoading: true, error: null });
        
        try {
          const response = await authService.loginAsVip(credentials);
          await setItem(APP_CONFIG.STORAGE_KEYS.AUTH_TOKEN, response.token);
          
          set({
            user: { ...response.user, isVip: true },
            isLoggedIn: true,
            isLoading: false,
            error: null,
          });
          ...
          set({
            isLoading: false,
            error: authError.message || 'VIP login failed',
          });
          throw error;
        }
      },
